...
**椭球按调用方按次指定**：正/反算请求均可带可选字段 `ellipsoid`，缺省为
**WGS84**，缺省路径与改造前逐位一致（含比例因子、收敛角）。椭球几何量
（b、e²、e′²、e1）由权威定义值（a、1/f）在每次请求时现推，喂给同一套
Krüger 级数；不设任何进程级椭球配置，同一服务实例可并发处理不同椭球请求。

- 内置椭球（权威定义值）：

  | 标识 | 名称 | 长半轴 a (m) | 扁率倒数 1/f |
  |---|---|---|---|
  | `WGS84`（缺省） | WGS84 / EPSG:7030 | 6378137.0 | 298.257223563 |
  | `GRS80` | GRS80 / EPSG:7019 | 6378137.0 | 298.257222101 |
  | `BESSEL_1841` | Bessel 1841 / EPSG:7004 | 6377397.155 | 299.1528128 |
  | `CLARKE_1866` | Clarke 1866 / NAD27 / EPSG:7008 | 6378206.4 | 294.9786982 |

  标识大小写不敏感，常见写法（`BESSEL`、`clarke 1866`、`NAD27`、
  `EPSG:7004` 等）均能识别。未收录的标识返回结构化错误
  `UNKNOWN_ELLIPSOID`，**绝不静默退回 WGS84**。
- UTM 层面约定与椭球无关，保持钉死：
  - 比例因子：中央经线 **k0 = 0.9996**
...
- 往返闭合精度：每种内置椭球全球随机点正算→反算，纬度误差 < 1×10⁻⁸ 度、
...
> 响应里的 `datum`（= 椭球规范标识）为兼容老调用方保留；`ellipsoid`
> 额外给出本次实际使用的定义值与派生量，供调用方核对“算的确实是这个球”。

**按次指定椭球**：请求体加 `"ellipsoid": "BESSEL_1841"` 即可，比例因子、
收敛角、坐标全部按该椭球的派生量重算；UTM 的 k0 与假偏移不变：

```bash
curl -X POST http://127.0.0.1:8080/api/v1/forward \
  -H 'content-type: application/json' \
  -d '{"lat": 39.9087, "lon": 116.3975, "ellipsoid": "BESSEL_1841"}'
# 北坐标 4417354.623 m（WGS84 为 4417797.609 m，差数百米）
```
...
**`ellipsoid` 必须与生成该平面坐标时所用的椭球一致**，否则属于物理上
不自洽的输入：东坐标跨带、北坐标越界等情形会被既有校验拦下。

```bash
curl -X POST http://127.0.0.1:8080/api/v1/inverse \
  -H 'content-type: application/json' \
  -d '{"zone":50,"easting":448502.182,"northing":4417797.609,"hemisphere":"N"}'
# → lat 39.9087, lon 116.3975（缺省 WGS84）

curl -X POST http://127.0.0.1:8080/api/v1/inverse \
  -H 'content-type: application/json' \
  -d '{"zone":50,"easting":448508.368,"northing":4417354.623,"hemisphere":"N","ellipsoid":"BESSEL_1841"}'
# → lat 39.9087, lon 116.3975（Bessel 1841）
```
...
分带查询是纯经度 → 带号映射，**与椭球几何无关**：即使传入 `ellipsoid`
字段也会被忽略，任何椭球下分带结果都相同。
...
| 椭球标识未收录（不静默退回 WGS84） | `UNKNOWN_ELLIPSOID` |
| 反算经纬度落回投影域之外 | `INVERSE_OUT_OF_DOMAIN` |

反算在校验平面坐标粗范围后，还会把反算得到的经纬度再做一次权威域校验，
拦截“勉强通过粗检但实际跨出投影带”的输入。北坐标粗检范围按**本次请求
椭球**的子午线弧长尺度现算（不同椭球的弧长不同），东坐标粗检范围属于
UTM 带宽几何约定，不随椭球变化。带号、平面坐标、椭球三者明显不自洽
（如东坐标落在别的带宽范围、北坐标超出该椭球弧长尺度）时，不会因为
多了椭球参数而绕过这些既有校验。
...
src/
  core/
    constants.js    UTM 投影约定（k0/假偏移/合法域，与椭球无关）
    ellipsoids.js   椭球权威定义值表（WGS84/GRS80/Bessel/Clarke）+ 按需派生量推导
    angles.js       唯一的角度换算点（度⇄弧度）
    errors.js       ValidationError 与类型化错误码表
    zones.js        分带规则、中央经线、带号合法性、强制带号判定
    meridian.js     子午线弧长 M(φ) 与反算底点纬度（按椭球现推弧长系数）
    tmForward.js    横轴墨卡托正算 Krüger 级数（式 8-13/14/15）+ 收敛角（椭球按次传入）
    tmInverse.js    横轴墨卡托反算 Krüger 级数（式 8-16..8-23，椭球按次传入）
    validation.js   全部输入/输出校验（含椭球标识、椭球感知的北坐标粗检）
    utm.js          门面：椭球解析+分带+正/反算+假偏移+强制带标注+示范点（纯函数 API）
  app.js            Fastify 路由与统一错误处理
  server.js         启动入口
test/               Node 内置 test runner，86 个用例
```

椭球相关职责独立放在 `ellipsoids.js`：级数模块（meridian/tmForward/
tmInverse）里**没有任何具体椭球的数值表**，只接收本次请求的椭球派生量；
新增椭球只需在 `ellipsoids.js` 的定义表里加一行权威 `a`、`1/f`。

### 并发安全

换算内核全部是**无状态纯函数**：椭球对象每次请求现推、用完即弃，
不写入任何全局位置、不做进程级椭球配置，模块加载后只有只读常量。
Fastify 单事件循环处理并发请求时，不同椭球请求各自独立、互不串扰
（`test/06-http.test.js` 含 300 个四种椭球混合并发请求的隔离测试，
`test/07-ellipsoids.test.js` 含交错椭球串行/并发一致性测试）。

---

## 正确性判据与测试

`npm test` 运行 86 个用例，覆盖需求中彼此独立、方向不同的各条判据：

1. **往返自洽（核心）**：全球格网点 + 500 个确定性随机点 + 带界接缝点，
   正算→反算还原经纬度，亚毫米闭合；反算→正算坐标层面也互逆
   （`03-roundtrip.test.js`）。正反级数任何一处系数写错都会在此漂移。
2. **多椭球（`07-ellipsoids.test.js`）**：
   - **默认路径逐位兼容**：不传 / 传 `WGS84` / 别名三种写法，东/北坐标、
     k、收敛角与改造前黄金值逐位相等；旧弧长系数与旧字面推导逐项相等；
   - **每种内置椭球分别往返闭合**：格网点 + 随机点 + 底点级数，亚毫米；
   - **同点不同椭球坐标必须不同**：非退化原点处 easting/northing/k/γ
     全部不同（北京点 Bessel 与 WGS84 北坐标差数百米）；退化原点
     （赤道+中央经线）对所有椭球同为 500000/0；
   - **k、γ 真走新派生量**：与“WGS84 几何 + 换标签”的伪造结果严格不同；
     级数弧长另用椭圆曲率半径的独立数值积分交叉核验；
   - **非法标识结构化报错**：`UNKNOWN_ELLIPSOID` / `INVALID_TYPE`，
     不退回 WGS84；
   - **带号/坐标/椭球不自洽**：各椭球下东坐标跨带、北坐标越界、
     半球矛盾分别按既有错误码拦下；北坐标粗检跨度按椭球弧长现算；
   - **分带与椭球无关**：传入椭球也被忽略；
   - **无状态并发**：四种椭球交错请求与串行参考结果逐值相同。
3. **赤道 / 中央经线基准**：(0°, λ0) 正算东坐标 = 500000、北坐标 = 0；
   中央经线上比例因子恒为 0.9996、东坐标恒为 500000
   （`02-forward-baseline.test.js`）。
4. **比例因子单调**：从中央经线向东偏离时 k 单调升高，东西对称点相等。
5. **南北半球配对**：南纬点北坐标 = 10000000 − 同经度北纬点北坐标。
6. **差一个带识别**：分带结果必须等于最近中央经线；用相邻错带正算时
   东坐标整整偏离约一个带宽（赤道 ≈ 668–672 km）；并对“分带公式被改成
   +2”的变异注入做了检出测试（`04-off-by-one-zone.test.js`）。
7. **子午线收敛角**：中央经线/赤道为零、东正西负、与一阶 γ≈Δλ·sinφ
   吻合。
8. **各类非法输入**：纬度/经度越界、带号非法、东/北坐标越界、半球非法、
   类型错误、缺字段、反算落出投影域，逐一断言不同错误码
   （`05-validation.test.js`）。
9. **HTTP 层**：全部接口的 POST/GET、ellipsoid 字段透传、结构化 400
   错误、非法 JSON、404、四种椭球混合并发隔离（`06-http.test.js`）。
